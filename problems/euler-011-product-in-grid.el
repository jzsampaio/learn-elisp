...
(defun withinBounds (xs i) "Return non-nill if length of list is greater than i" (and (length> xs i) (>= i 0)))
...
(defun getAllDirections (xs i j)
     (list (getVertical4 xs i j) (getHorizontal4 xs i j) (getDiagonal xs i j) (getDiagonalUp xs i j))
     )

(defun mapij (FUNCTION xxs)
  (seq-map-indexed
   (lambda (xs i)
     (seq-map-indexed (
		       lambda (x j)
			      (funcall FUNCTION x i j)
			      )
		      xs)
     )
   xxs)
...
(let*
    (
     (allDirections (mapij (lambda (x i j) (flatten-list (getAllDirections DATA i j))) DATA))
     (answer (apply 'max (flatten-list ANS)))
     )
  (message "Answer: %d" answer)
  )
